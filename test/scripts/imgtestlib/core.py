...
def export_pipeline_from_build_request(manifest_data) -> str:
    export_pipeline = manifest_data.get("build-request", {}).get("export-pipeline")
    if not export_pipeline:
        raise ValueError("build-request missing export-pipeline")
    return export_pipeline


def export_pipeline_shell_flag(export_pipeline: str) -> str:
    return f' --export-pipeline "{export_pipeline}"'
...
def read_manifests(path):
    ...
    for manifest_fname in os.listdir(path):
        ...
        export_pipeline = export_pipeline_from_build_request(manifest_data)
...
def filter_builds(manifests, distro=None, arch=None, skip_ostree_pull=True):
    ...
    for manifest_fname, entry in manifests.items():
        ...
        export_pipeline = entry["export-pipeline"]
