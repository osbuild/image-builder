...
def ensure_env_vars(env_vars: Dict[str, Optional[str]]):
    missing = [name for name, value in env_vars.items() if value is None or value == ""]
    if missing:
        raise RuntimeError(f"Missing or empty environment variables: {missing}")
...
def boot_qemu_iso_no_unattended_support(distro, arch, image_type, installer_iso_path, config_file,
                                        iso_embedded_ks_path=None):
    """
    Boot an ISO that has no "unattended" support in its blueprint.
    Manually create a custom kickstart file and modify the ISO to use it.
    """
    # If an embedded kickstart was provided, prepend its content to preserve original directives
    # (unless overridden by the unattended automation directives added below).
    custom_ks_content = None
    if iso_embedded_ks_path:
        with open(iso_embedded_ks_path, "r", encoding="utf-8") as fp:
            custom_ks_content = fp.read()
        print(f"ISO embedded kickstart content: \n{custom_ks_content}\n\n")

    rootpw = "".join(random.choices(string.ascii_uppercase + string.digits, k=18))
    rhsm = ""
    rhsm_unregister = ""

    # NOTE: we do not need to register the bootc image, since all content comes from the container
    if distro.startswith("rhel") and not image_type.startswith("bootc-"):
    ...
    with contextlib.ExitStack() as cm:
        ...
        ks_content = ""
        if custom_ks_content:
            ks_content += "# Content of the original ks embedded in the ISO\n"
            ks_content += custom_ks_content + "\n\n"

        ks_content += textwrap.dedent(f"""\
        # Unattended automation directives generated by boot_qemu_iso_no_unattended_support()
        ...
        # with our "check-host-config.sh" that expects a non-degraded boot
        systemctl mask mcelog.service || true
        echo "osbuild ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/osbuild
        chmod 0440 /etc/sudoers.d/osbuild
        {rhsm_unregister}
        %end
        """)
        unattended_ks.write_text(ks_content)
