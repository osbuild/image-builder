...
func TestLoadAllowList(t *testing.T) {
	mockLogger, err := logger.NewLogger("DEBUG", "", "", "", "", "")
	require.NoError(t, err)

	t.Run("no allow file", func(t *testing.T) {
		actual, err := LoadAllowList("", mockLogger)
		expected := AllowList{}
		require.Nil(t, err)
		require.Equal(t, expected, actual)
	})
	...
	t.Run("allow file does not exist", func(t *testing.T) {
		actual, err := LoadAllowList("testdata/nonexistantfile.json", mockLogger)
	...
	t.Run("valid allowFile exists", func(t *testing.T) {
		actual, _ := LoadAllowList("testdata/allow.json", mockLogger)
